...
def mostrar_frases(frases:list, porcentaje:float = 0.8)-> None:
    ...
    for lista in frases:
        ...
        ratio = lista[2]
        if ratio >= porcentaje:
            resultados.append([frase, pelicula, ratio])
    resultados.sort(key=lambda x:x[2], reverse=True)
    for lista in resultados:
        frase = lista[0]
        pelicula = lista[1]
        ratio = lista[2]
        print(f"Frase: {frase} - Pelicula: {pelicula} - Ratio: {ratio:.2f}")        
...
if __name__ == "__main__":
    #python frases_levenshtein.py --archivo frases.csv --frase "El futuro nuestro es" --tasa 0.8

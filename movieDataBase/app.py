...
def agregar_relacion():
    ...
    if request.method == 'GET':
        ...
        for actor in sistema.actores.values():
            actores_list.append({'id_estrella': actor.id_estrella, 'nombre': actor.nombre})
            sorted_actores = sorted(actores_list, key=lambda x: x['nombre'])
        peliculas_list=[]
        for pelicula in sistema.peliculas.values():
            peliculas_list.append({'id_pelicula': pelicula.id_pelicula, 'titulo_pelicula': pelicula.titulo_pelicula})
            sorted_peliculas = sorted(peliculas_list, key=lambda x: x['titulo_pelicula'])
    ...
    if request.method == 'POST':
            ...
            sistema.guardar_csv(relaciones_csv, mc.Relacion)
